...
import { getAdminInventoryUseCase, updatePackStockUseCase, packRepository, userRepository } from '../../core/container';

type AdminFilter = 'todos' | 'agotados' | 'publicados';

export const AdminDashboard: React.FC = () => {
  const [packs, setPacks] = useState<Pack[]>([]);
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [activeFilter, setActiveFilter] = useState<AdminFilter>('todos');
  const [stockFlashId, setStockFlashId] = useState<string | null>(null);
  ...
  const handleUpdateStock = async (packId: string, delta: number) => {
    ...
    try {
      ...
      // Flash effect
      setStockFlashId(packId);
      setTimeout(() => setStockFlashId(null), 600);
    } catch (err) {
  ...
  const handleCreatePack = async (e: React.FormEvent) => {
    ...
    // Validations
    const name = sanitizeText(newPack.name);
    const description = sanitizeText(newPack.description);
    ...
    if (Number(newPack.originalPrice) <= 0) {
      setFormError('El precio original debe ser mayor a $0.');
      return;
    }
    if (Number(newPack.discountedPrice) <= 0) {
      setFormError('El precio de descuento debe ser mayor a $0.');
      return;
    }
    if (Number(newPack.discountedPrice) >= Number(newPack.originalPrice)) {
      setFormError('El precio de descuento debe ser menor que el precio original.');
      return;
    }
    if (Number(newPack.stock) <= 0) {
      setFormError('El stock inicial debe ser al menos 1.');
      return;
    }
    ...
    try {
      await packRepository.addPack({
        ...
        imageUrl: newPack.category === 'panaderia' 
          ...
          : newPack.category === 'saludable'
            ? 'https://images.unsplash.com/photo-1512621776951-a57141f2eefd?ixlib=rb-4.0.3&auto=format&fit=crop&w=400&q=80'
            : 'https://images.unsplash.com/photo-1512152272829-e3139592d56f?ixlib=rb-4.0.3&auto=format&fit=crop&w=400&q=80',
      ...
      setFormError('');
      loadInventory();
      showToast('¡Pack creado y publicado exitosamente! 🎉');
    } catch (err) {
      console.error('Error creando pack:', err);
      showToast('Error al crear el pack', 'error');
    } finally {
      setIsCreating(false);
    }
  };
  ...
  const filteredPacks = packs.filter(pack => {
    if (activeFilter === 'agotados') return pack.stock === 0;
    if (activeFilter === 'publicados') return pack.stock > 0;
    return true;
  });
    ...
    <div className="view-section px-4 sm:px-6 lg:px-8 mt-24 mb-24">
      ...
      {toast && (
        <div className="fixed top-6 left-1/2 -translate-x-1/2 z-[1100] w-full max-w-sm px-4 animate-slide-up-modal">
          <div className={`${toast.type === 'success' ? 'bg-emerald-600 border-emerald-400' : 'bg-red-600 border-red-400'} text-white p-3.5 rounded-2xl shadow-xl border-2 flex items-center gap-3`}>
            <i className={`fa-solid ${toast.type === 'success' ? 'fa-circle-check' : 'fa-circle-exclamation'} text-xl`} />
            <p className="text-sm font-bold flex-1">{toast.message}</p>
            <button onClick={() => setToast(null)} className="text-white/80 hover:text-white" aria-label="Cerrar">
      ...
      <div className="flex items-center justify-between mb-6 md:mb-8 bg-white dark:bg-gray-800 p-4 md:p-6 rounded-[1.5rem] shadow-sm border border-gray-100 dark:border-gray-700">
        ...
        <button onClick={handleLogout} className="text-sm font-bold text-gray-500 dark:text-gray-400 hover:text-red-500 dark:hover:text-red-400 flex items-center bg-gray-50 dark:bg-gray-700 py-2.5 px-4 rounded-xl transition" aria-label="Cerrar sesión">
          <i className="fa-solid fa-arrow-right-from-bracket mr-2" /> <span>Cerrar Sesión</span>
        </button>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        ...
        <div 
          ...
          className="bg-gradient-to-br from-gray-900 to-gray-800 p-5 rounded-2xl border border-gray-700 shadow-sm flex flex-col justify-center text-white cursor-pointer hover:shadow-lg transition active:scale-95"
          role="button"
          aria-label="Crear nuevo pack"
        >
          <div className="text-gray-300 text-[10px] md:text-xs font-bold uppercase tracking-wider mb-1">Acción rápida</div>
          <div className="text-lg md:text-xl font-extrabold flex items-center"><i className="fa-solid fa-plus-circle mr-2 text-emerald-500" /> Nuevo Pack</div>
        </div>
      </div>
      ...
      <div className="flex justify-between items-center mb-5">
        ...
        <div className="flex gap-2" role="tablist" aria-label="Filtrar inventario">
          {([
            { key: 'todos' as AdminFilter, label: 'Todos' },
            { key: 'publicados' as AdminFilter, label: 'Publicados' },
            { key: 'agotados' as AdminFilter, label: 'Agotados' },
          ]).map(filter => (
            <button
              key={filter.key}
              ...
              className={`text-xs font-bold px-3 py-1.5 rounded-lg cursor-pointer transition ${
            ...
            >
              {filter.label}
              {filter.key === 'agotados' && packs.filter(p => p.stock === 0).length > 0 && (
                <span className="ml-1.5 bg-red-500 text-white text-[9px] w-4 h-4 rounded-full inline-flex items-center justify-center">
                  {packs.filter(p => p.stock === 0).length}
                </span>
              )}
            </button>
          ))}
      ...
      {filteredPacks.length === 0 ? (
        <div className="text-center py-12 bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-3xl">
          <div className="text-4xl mb-3">{activeFilter === 'agotados' ? '✅' : '📦'}</div>
          <p className="text-gray-500 dark:text-gray-400 font-bold">
            {activeFilter === 'agotados' ? '¡Genial! Ningún pack está agotado' : 'No hay packs en esta categoría'}
          </p>
        </div>
      ) : (
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex items-center bg-gray-50 dark:bg-gray-900 rounded-lg p-1 border border-gray-100 dark:border-gray-700">
                    <button 
                      ...
                      className="w-8 h-8 flex items-center justify-center text-gray-500 dark:text-gray-400 font-bold rounded-md hover:bg-white dark:hover:bg-gray-800 hover:shadow-sm transition active:scale-90"
                      aria-label={`Reducir stock de ${pack.name}`}
                    >
                      -
                    </button>
                    <span className={`w-8 text-center font-extrabold text-gray-900 dark:text-white text-lg transition-colors ${stockFlashId === pack.id ? 'animate-stock-flash' : ''}`}>
                      {pack.stock}
                    </span>
                    <button 
                      onClick={() => handleUpdateStock(pack.id, 1)}
                      className="w-8 h-8 flex items-center justify-center text-gray-500 dark:text-gray-400 font-bold rounded-md hover:bg-white dark:hover:bg-gray-800 hover:shadow-sm transition active:scale-90"
                      aria-label={`Aumentar stock de ${pack.name}`}
                    >
                      +
                    </button>
                  </div>
      ...
      {/* MODAL CREAR PACK */}
      {isModalOpen && (
        <div className="fixed inset-0 z-[1000] flex items-end md:items-center justify-center modal-overlay animate-show" onClick={() => !isCreating && setIsModalOpen(false)}>
          <div className="bg-white dark:bg-gray-800 md:rounded-3xl rounded-t-3xl border border-gray-100 dark:border-gray-700 p-6 md:p-8 max-w-md w-full shadow-2xl animate-slide-up-modal" onClick={e => e.stopPropagation()}>
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-xl font-extrabold text-gray-900 dark:text-white font-display">Crear Nuevo Pack</h3>
              <button onClick={() => { setIsModalOpen(false); setFormError(''); }} disabled={isCreating} className="text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 disabled:opacity-50" aria-label="Cerrar modal">
                <i className="fa-solid fa-xmark text-lg" />
              </button>
            </div>

            {formError && (
              <div className="mb-4 p-3 bg-red-50 dark:bg-red-900/20 border border-red-100 dark:border-red-900/30 text-red-600 dark:text-red-400 rounded-xl text-xs font-bold flex items-center gap-2 animate-show">
            ...
            <form onSubmit={handleCreatePack} className="space-y-4">
              ...
              <div>
                ...
                <textarea 
                  ...
                  className="w-full bg-gray-50 dark:bg-gray-900 text-gray-800 dark:text-white border border-gray-200 dark:border-gray-700 rounded-xl p-3 outline-none focus:border-emerald-500 font-medium text-sm h-20 resize-none"
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  />
                  {Number(newPack.discountedPrice) >= Number(newPack.originalPrice) && Number(newPack.discountedPrice) > 0 && (
                    <p className="text-red-500 text-[10px] font-bold mt-1">Debe ser menor al precio original</p>
                  )}
                </div>
              </div>
              ...
              <div>
                ...
                <input 
                  ...
                  className="w-full bg-gray-50 dark:bg-gray-900 text-gray-800 dark:text-white border border-gray-200 dark:border-gray-700 rounded-xl p-3 outline-none focus:border-emerald-500 font-medium text-sm"
                  required
                  aria-label="Horario de recogida"
                />
              </div>
              ...
              <div className="flex items-center gap-2 pt-2">
                <input 
                  ...
                  className="w-4 h-4 text-emerald-600 focus:ring-emerald-500 border-gray-300 rounded accent-emerald-500"
                />
              ...
              <button 
                type="submit"
                disabled={isCreating}
                className="w-full btn-primary text-white font-bold py-3.5 px-6 rounded-xl shadow-md active:scale-95 transition-all uppercase tracking-wide text-sm mt-4 flex items-center justify-center gap-2 disabled:opacity-60 disabled:cursor-not-allowed"
              >
                {isCreating ? (
